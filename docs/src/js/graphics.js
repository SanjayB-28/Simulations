...
function drawPump(x, y, size, pipeWidth, label) {
  ...
  if (label === "Tank B") {
    ...
    // Draw the tank
    drawSimpleTank(p, tankX, tankY, tankWidth, tankHeight, finalTankColor); // Use blended color
  }
...
function drawSimpleTank(p, x, y, width, height, color) {
  // Draw tank outline
  p.stroke(0);
  p.noFill();
  p.rect(x - width/2, y - height/2, width, height);
  ...
  p.fill(color);
  p.noStroke();
  p.rect(x - width/2, y + height/2 - height * 0.8, width, height * 0.8);
}
